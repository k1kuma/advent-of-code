...
// for validity.
function isValidPassport(fieldMap) {
  ...
  // shouldn't reach here but log out 
  console.log('SHOULD NOT HAVE REACHED HERE');
  return false;
}

function validatePassport(fieldMap) {
  if (isValidPassport(fieldMap)) {
    console.log('VALID PASSPORT FOUND');
    numValid++;
  }
  fieldMap = new Map();
  return;
...
lineReader.eachLine('input.txt', function(line,last) {
  ...
  for (let i = 0; i < tokens.length; i ++) {
    ...
    if (isExpectedField(element) && !fieldMap[element]) {
  ...
  if (last) {
    validatePassport(fieldMap);  // ;ast line may mean not running into blank line to validate
